...
<div class="container-fluid py-4 invoice-page">
    ...
    <div class="page-header">
        <div class="row align-items-center">
            ...
            <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                ...
                <a href="{{ route('invoices.create') }}" class="btn-modern btn-modern-primary">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card info">
            ...
            <div class="stat-value">{{ number_format($invoices->sum('paid_amount'), 2) }}</div>
        ...
        <div class="stat-card danger">
            ...
            <div class="stat-value">{{ number_format($invoices->sum('balance'), 2) }}</div>
    ...
    <div class="toolbar">
        ...
        <div class="filter-group">
            ...
            <span class="results-badge">
                {{ $invoices->count() }} / {{ number_format($invoices->total()) }}
            </span>
        </div>
    </div>
                ...
                <tbody>
                    @forelse($invoices as $invoice)
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="cell-primary">#{{ $invoice->invoice_no }}</div>
                                <div class="cell-secondary">ID: {{ $invoice->id }}</div>
                            </td>
                                ...
                                <div class="student-cell">
                                    ...
                                    <div>
                                        <div class="fw-semibold text-dark">
                                            {{ $invoice->student->first_name ?? 'N/A' }}
                                            {{ $invoice->student->last_name ?? '' }}
                                        </div>
                                        <div class="cell-secondary">
                                            {{ $invoice->student->admission_no ?? 'No Admission No' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                @if($studentType)
                                    <span class="type-badge {{ $typeClass }}">
                                        <i class="bi bi-person-{{ $studentType == 'New' ? 'plus' : 'check' }}"></i>
                                        {{ $studentType }}
                                    </span>
                                @else
                            ...
                            <td>
                                <div class="fee-tags">
                                    @foreach($invoice->invoiceItems->take(2) as $item)
                                        <span class="fee-tag">{{ $item->feeCategory->name ?? 'N/A' }}</span>
                                    @endforeach
                                    @if($invoice->invoiceItems->count() > 2)
                                        <span class="fee-more">+{{ $invoice->invoiceItems->count() - 2 }}</span>
                                    @endif
                                </div>
                            </td>
                            
                            <td class="text-end">
                                <span class="amount amount-total">{{ number_format($invoice->total_amount ?? 0, 2) }}</span>
                            </td>
                            
                            <td class="text-end">
                                <span class="amount amount-paid">{{ number_format($invoice->paid_amount ?? 0, 2) }}</span>
                            </td>
                            
                            <td class="text-end">
                                <span class="amount amount-balance">{{ number_format($invoice->balance ?? 0, 2) }}</span>
                            </td>
                            ...
                            <td>
                                <div class="fw-semibold">{{ optional($invoice->created_at)->format('d M Y') }}</div>
                                <div class="cell-secondary">{{ optional($invoice->created_at)->diffForHumans() }}</div>
                            </td>
        ...
        @if($invoices->hasPages())
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Showing <strong>{{ $invoices->firstItem() ?? 0 }}</strong> 
                    to <strong>{{ $invoices->lastItem() ?? 0 }}</strong> 
